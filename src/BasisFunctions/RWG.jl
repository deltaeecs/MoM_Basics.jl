...
bfID   :   基函数编号，整形
edgel   :   基函数边长，浮点型
inGeo   :   基函数所在两个三角形编号（半基函数为1个，赋值成一样的两个），长度为2的向量数组
inGeo   :   基函数在两个上面三角形中的局部编号（半基函数为1个，赋值成一样的两个），取值1:3，长度为2的向量数组
center  :   边的中点
"""
...
# import Base: zero
# zero(::Type{RWG{IT, FT}}) where{IT<:Integer, FT<:AbstractFloat} = RWG{IT, FT}()
...
采用排序算法，将每个边的两个点、在三角形中的对点、三角形、基函数等属性信息放在一个大数组中，
通过对不同的属性排序（如按边所在点随数组排序即可将边相同的点放在一起），即可得到基函数分组信息，以此可构造RWG基函数。
函数完成以下功能：
1. 构造基函数类型实例数组（类似结构化数组）rwgsInfo记录基函数相关信息，并作为返回值；
2. 写入三角形类型实例数组trianglesInfo中关于基函数的信息。
